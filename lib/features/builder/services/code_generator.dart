...
class CodeGenerator {
  static String generateMainDartCode(List<WidgetData> widgets, String appName) {
    ...
    // Generate AppBar if exists
    final appBarWidgets = widgets.where((w) => w.type == WidgetData.TYPE_APPBAR).toList();
    if (appBarWidgets.isNotEmpty) {
      code.writeln("      appBar: ${_generateWidgetCode(appBarWidgets.first)},");
    }
    
    // Generate body
    final bodyWidgets = widgets.where((w) => w.type != WidgetData.TYPE_APPBAR).toList();
    if (bodyWidgets.isNotEmpty) {
      code.writeln("      body: ${_generateWidgetCode(bodyWidgets.first)},");
    } else {
  ...
  static String generateHomePageCode(List<WidgetData> widgets, String appName) {
    final StringBuffer code = StringBuffer();
    ...
    // HomePage class
    code.writeln("class HomePage extends StatefulWidget {");
    code.writeln("  @override");
    code.writeln("  _HomePageState createState() => _HomePageState();");
    code.writeln("}");
    code.writeln();
    
    // State class
    code.writeln("class _HomePageState extends State<HomePage> {");
    
    // Controllers and variables
    code.writeln("  // Controllers");
    _generateControllers(code, widgets);
    code.writeln();
    ...
    code.writeln("        title: Text('$appName'),");
    ...
    if (widgets.isEmpty) {
      ...
      code.writeln("          Icon(Icons.home, size: 64, color: Colors.grey),");
      code.writeln("          SizedBox(height: 16),");
      code.writeln("          Text('Welcome to $appName', style: TextStyle(fontSize: 24)),");
    ...
    _generateWidgetMethods(code, widgets);
  ...
  static void _generateControllers(StringBuffer code, List<WidgetData> widgets) {
    ...
    for (int i = 0; i < editTextWidgets.length; i++) {
      ...
      final controllerName = '_${widget.id}Controller';
      code.writeln("  final TextEditingController $controllerName = TextEditingController();");
    }
  }

  static void _generateWidgetMethods(StringBuffer code, List<WidgetData> widgets) {
  ...
  // Generate JSON representation of widgets for storage
  static String generateWidgetsJson(List<WidgetData> widgets) {
    final List<Map<String, dynamic>> widgetsJson = widgets.map((w) => w.toJson()).toList();
    return jsonEncode(widgetsJson);
  }

  // Parse JSON back to widget data
  static List<WidgetData> parseWidgetsFromJson(String json) {
    final List<dynamic> widgetsJson = jsonDecode(json);
    return widgetsJson.map((w) => WidgetData.fromJson(w)).toList();
  }
} 